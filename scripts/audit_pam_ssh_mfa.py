...
def audit_pam_sshd(pam_text: str) -> List[CheckResult]:
    ...
    has_google = bool(re.search(r"^\s*auth\s+.*pam_google_authenticator\.so\b", pam_text, re.M))
    checks.append(CheckResult(
        id="pam_google_authenticator_present",
        description="/etc/pam.d/sshd includes pam_google_authenticator.so in auth stack",
        passed=has_google,
        details="Found" if has_google else "Missing 'pam_google_authenticator.so' in auth stack",
    ))

    # Prompt order heuristic: ensure GA line appears before common-auth include (common in Debian-like systems)
    ...
    if ga_idx is not None and common_auth_idx is not None:
        ...
        details = f"pam_google_authenticator line={ga_idx+1}, common-auth include={common_auth_idx+1}"
    else:
        passed = True
        details = "Could not evaluate prompt-order heuristic (non-Debian layout or missing markers)."
    checks.append(CheckResult(
    ...
    has_access = bool(re.search(r"^\s*account\s+.*pam_access\.so\b", pam_text, re.M))
    checks.append(CheckResult(
        id="pam_access_present",
        description="/etc/pam.d/sshd includes pam_access.so in account stack",
        passed=has_access,
        details="Found" if has_access else "Missing 'pam_access.so' in account stack",
    ))

    has_time = bool(re.search(r"^\s*account\s+.*pam_time\.so\b", pam_text, re.M))
    checks.append(CheckResult(
        id="pam_time_present",
        description="/etc/pam.d/sshd includes pam_time.so in account stack (optional)",
        passed=has_time,
...
def audit_sshd_config(sshd_text: str) -> List[CheckResult]:
    ...
    def has_directive(key: str, expected: Optional[str] = None) -> Tuple[bool, str]:
        ...
        return (val.lower() == expected.lower()), f"{key} {val} (expected {expected})"
    ...
    checks.append(CheckResult(
        ...
        description="sshd_config: interactive auth enabled (KbdInteractiveAuthentication/ChallengeResponseAuthentication)",
        passed=passed,
        details=details,
    ))

    # PasswordAuthentication may be optional if you use pubkey+TOTP only; treat optional by default.
    ok, details = has_directive("PasswordAuthentication", "yes")
    checks.append(CheckResult(
        id="sshd_passwordauth_yes",
        description="sshd_config: PasswordAuthentication yes (optional depending on 2FA design)",
        passed=ok,
        details=details,
        severity="optional",
    ))
    ...
    if not m:
        ...
        am = m.group(1).strip()
        needs_interactive = bool(re.search(r"keyboard-interactive|kbdinteractive", am, re.I))
        checks.append(CheckResult(
            id="sshd_authenticationmethods_includes_interactive",
            description="sshd_config: AuthenticationMethods includes keyboard-interactive (recommended)",
...
def audit_access_conf(access_text: str, expected_rules: Optional[int]) -> List[CheckResult]:
    checks: List[CheckResult] = []
    ...
    if expected_rules is not None:
        checks.append(CheckResult(
            ...
            description=f"access.conf has expected number of rules ({expected_rules})",
    ...
    passed = bool(last_rule_line and re.match(r"^-\s*:\s*ALL\s*:\s*ALL\s*$", last_rule_line, re.I))
    checks.append(CheckResult(
...
def main() -> int:
    ...
    ap.add_argument("--root", default="/", help="Alternate root (for mounted images / fixtures)")
    ap.add_argument("--json", action="store_true", help="Emit JSON output")
    ap.add_argument("--expected-access-rules", type=int, default=None, help="Optional expected rule count in access.conf")
    args = ap.parse_args()
